...
**Greedy Approach:** The idea is to subtract the largest possible Roman numeral values from the given integer and append the corresponding symbols to the result string. This process continues until the entire number is reduced to zero.

1. Create an array `romanMap` that contains all possible values and their corresponding symbols in descending order. This array should include both the normal and subtractive values.
2. Initialize an empty string `result` to store the Roman numeral.
3. For each value-symbol pair in `romanMap`, check how many times the value can be sutracted from the input integer (using `Math.floor(num/value)`).
4. Append the corresponding symbol to `result` and decrease the integer by the subtracted value (`num-=value`).
5. Repeat the process until the integer becomes zero.
